...
function renderChatList() {
  ...
  state.chats.forEach((chat) => {
    ...
    button.innerHTML = '<span class="avatar">' + initials(user) + '</span><span><span class="item-title"><span>' + escapeHtml(otherName) + '</span><time>' + formatTime(chat.updatedAt) + '</time></span><span class="item-preview">' + escapeHtml(chat.lastMessage || "Chat open karein") + '</span></span>';
...
function renderActiveChat() {
  ...
  els.activeMeta.textContent = hasChat ? "ID: " + state.activeUser.id + " | Firebase real-time chat" : "Firebase real-time private thread ready hoga.";
...
auth.onAuthStateChanged((user) => {
  ...
  if (state.currentUserId) listenToChats();
  renderShell();
});
