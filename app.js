...
function setReply(message) {
  ...
  if (els.replyStrip) {
    els.replyStrip.hidden = !state.replyTo;
    if (state.replyTo && els.replyText) els.replyText.textContent = state.replyTo.text;
  }
}
...
function renderShell() {
  ...
  if (els.searchPanel) els.searchPanel.hidden = !state.searchOpen;
  if (els.pinChat) {
...
function renderActiveChat() {
  ...
  if (els.replyStrip) els.replyStrip.hidden = !state.replyTo || !hasChat;
  if (!hasChat) {
    if (els.typingStrip) {
      els.typingStrip.hidden = true;
      els.typingStrip.textContent = "";
...
function renderUsers() {
  ...
  if (!state.searchOpen) return appendHint(els.searchResults, "Search khol kar user ID type karein.");
...
els.pinChat.addEventListener("click", () => togglePinnedChat(state.activeChatId));
